...
def load_and_preprocess_data():
    ...
    merged_data = pd.merge(movies, ratings, on='movieId').drop(['genres', 'timestamp'], axis=1)
    user_ratings = merged_data.pivot_table(index='userId', columns='title', values='rating')
...
def get_similar_movies(movie_name, n_recommendations=10):
    ...
    scores = similarity_matrix[movie_name].sort_values(ascending=False)[1:n_recommendations+1]
    # Get movie titles
...
def autocomplete():
    ...
    suggestions = movies_df[movies_df['title'].str.contains(query, case=False, na=False)]['title'].tolist() if query else []
...
def recommend():
    ...
    ranked_recommendations = [{"rank": i+1, "title": title} for i, title in enumerate(recommendations)]
